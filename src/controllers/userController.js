...
const {
  ...
  dupliChecknameService,
} = userService;

const signUpController = async (req, res) => {
  try {
    const { name, email, password, profileImage, phoneNumber, birthday } =
      req.body;
rmSync
    //필수 값 확인 - falsy 사용
    if (!name || !email || !password ||!phoneNumber)
      throwError(400, 'missing name, email or password');
...
const dupliCheckController = async (req, res) => {
  ...
  try {
    ...
    if (email) {
      ...
      const check = await dupliChecknameService(name);
      if (check > 0) {
        return res.status(400).json({ message: 'name is Already in Use' });
      } else {
        return res.status(200).json({ message: 'name can be used' });
      }
    }
  } catch (err) {
    console.log(err);
